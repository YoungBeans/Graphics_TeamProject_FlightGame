...
public class Charactor_Fire : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider collider)
    {
        ...
        if (collider.GetComponent<Collider>().CompareTag("Player_Missile"))
        {
            // 메모리풀을 정리한다.
    ...
    // 플레이어와의 거리를 측정하여 미사일 발사 여부를 결정
    void DistanceChecker()
    {

        if ((this.transform.position.y < 40f)& (this.transform.position.y > -36f) & (this.transform.position.x < 23f) & (this.transform.position.x > -23f))
        {
            // 미사일 발사 허용
            FireEnabled = true;
        }
    ...
    void MissileFire()
    {
        ...
        if (FireEnabled)
        {
            ...
            for (int i = 0; i < MissileMaximumPool; i++)
            {
                if (Missiles[i])
                {
                    if (Missiles[i].transform.position.y < -36f | Missiles[i].transform.position.y > 42f | Missiles[i].transform.position.x > 42f)
                    {
